---
title: 1. 从 NSObject 的初始化了解 isa（转载)
date: 2018-11-01 10:38:11
tags: isa
categories: Runtime
toc: true
---
本文讲解Runtime中的isa指针，本文全部抄袭，  代码位于《objc-private.h》文件
...
所有的OC类其实都是一个c语言的结构体，我们可以通过clang -rewrite-objc 命令重写OC类为C++类。来理解OC的类就是一个C语言的结构体。下面是重写的实例，类的定义如下：

```
@interface LJObjc:NSObject
...
@implementation LJObjc

@end
```

在从重写后的输出文件中，我们找到LJObjc的定义，定义如下：

```
typedef struct objc_object LJObjc;
```

可以看出，OC类就是一个C的结构体。

既然所有的OC对都是一个结构体，这个结构体就是struct objc_object。定义如下：
...
objc_object结构中只有一个 `isa_t`类型成员。所以我们可以说含有isa的结构体都是一个对象。

> 所有继承自 `NSObject` 的类实例化后的对象都会包含一个类型为 `isa_t` 的结构体。

虽然上面重写LJObjc后的C语言结构体是一个objc_object对象，但实际上他是一个objc_class结构体，由于objc_class继承与objc_object，  所以本质上objc_class 也是一个objc_object结构。
objc_class定义如下：
...
![isa 指针-- 实例找到类](1_NSObject_isa/objc-isa-class-pointer.png)

但是，这样就有一个问题，类方法的实现又是如何查找并且调用的呢？这时，就需要引入*元类*来保证无论是类还是对象都能**通过相同的机制查找方法的实现**。

![元类的引入](1_NSObject_isa/objc-isa-meta-class.png)
...
![元类](1_NSObject_isa/objc-isa-class-diagram.png)
...
![isa类图](1_NSObject_isa/objc-isa-class-object-class.png)
...
![isa各个位域的含义](1_NSObject_isa/objc-isa-isat.png)
...
![objc-isa-isat-bits](1_NSObject_isa/objc-isa-isat-bits.png)
...
![objc-isa-isat-bits-has-css-dto](1_NSObject_isa/objc-isa-isat-bits-has-css-dtor.png)
...
![objc-isa-print-cls](1_NSObject_isa/objc-isa-print-cls.png)
...
![objc-isa-print-object](1_NSObject_isa/objc-isa-print-object.png)
...
![objc-isa-print-class-object](1_NSObject_isa/objc-isa-print-class-object.png)
...
![objc-isa-isat-class-highlight-bits](1_NSObject_isa/objc-isa-isat-class-highlight-bits.png)
...
1. 对象： 含有isa指针的结构都是对象，包括类，实例。
2. isa指针的作用之一，统一类方法和实例方法的调用机制
